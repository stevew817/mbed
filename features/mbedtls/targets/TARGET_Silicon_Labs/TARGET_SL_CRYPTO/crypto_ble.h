 ...
 * \param length      Amount of keys in the keyTable array
 * \param prand       BLE nonce to encrypt with each key and match against hash
 * \param hash        BLE RPA hash to match against (last 24 bits of AES result)
 * \param[out] match  If we match one of the devices, the keytable index of that
 *                    device will be put in this variable.
 *
 * \return            0 if a match is found, -1 for no match.
 */
